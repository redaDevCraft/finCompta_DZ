...
export default function ClientsIndex({ clients, filters = {}, kpis = {} }) {
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        <h1 className="flex items-center gap-2 text-2xl font-semibold text-slate-900">
                            <Users className="h-6 w-6 text-indigo-600" /> Clients
                        </h1>
                        <p className="mt-1 text-sm text-slate-600">
                            {clients.total} client{clients.total > 1 ? 's' : ''} — encours client non lettré: <strong>{formatDzd(kpis.open_receivable)}</strong>
                        </p>
                    </div>
                    <Link
                        ...
                        <table className="min-w-full divide-y divide-slate-200 text-sm">
                            <thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500">
                                <tr>
                                    ...
                                    <th className="px-4 py-3 text-left">
                                        <button onClick={() => toggleSort('nif')} className="inline-flex items-center gap-1 hover:text-slate-900">
                                            NIF <ArrowUpDown className="h-3 w-3" />
                                        </button>
                                    </th>
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {clients.data.length ? clients.data.map((c) => (
                                    <tr key={c.id} className="hover:bg-slate-50">
                                        <td className="px-4 py-3">
                                            <Link href={route('clients.show', c.id)} className="font-medium text-indigo-700 hover:underline">
                                                {c.display_name}
                                            </Link>
                                            {c.raison_sociale && c.raison_sociale !== c.display_name && (
                                                <div className="text-xs text-slate-500">{c.raison_sociale}</div>
                                            )}
                                        </td>
                                        ...
                                        <td className="px-4 py-3 text-right font-semibold">{formatDzd(c.invoices_total)}</td>
                                    </tr>
                                )) : (
                                    <tr>
                            ...
                            {clients.links.map((link, i) => (
                                <button
                                    ...
                                    className={`rounded-lg px-3 py-1.5 text-xs ${
                                        link.active ? 'bg-indigo-600 text-white' : 'border border-slate-300 bg-white text-slate-700'
